...
/////////////////////////////////////////////
// Exact total bytes this function's swaps would move over a real network
// (party j's c[j][l][i] to party i, and party i's c[i][l][j] to party j —
// a genuine pairwise exchange, simulated here via memcpy since every
// party's state lives in this one process during offline setup). Computed
// via sizeof() on the actual struct, not a hand-derived formula.
size_t mal_setup_reindex_bytes(void){
    return (size_t)(CONST_N * (CONST_N - 1) / 2) * LAMBDA * 2 * sizeof(RSS_i[TAU_i * TAU_i]);
}
...
int main(int argc, char const *argv[])
    {
        ...
        // Exact per-party byte size of the (simulated, communication-free
        // here) trusted-party setup distribution — sizeof() on the actual
        // structures mal_tp_setup fills in for this party, so it reflects
        // the real field size (NBYTES_FIELD) and combinatorics (TAU, TAU_i)
        // exactly, with no hand-derived/approximated formula. In a real
        // deployment (no trusted dealer, PKI-distributed instead) this is
        // the one-time setup payload each party would receive; charged here
        // once per query since this benchmark harness regenerates it fresh
        // on every server restart.
        char tp_filename[256];
        snprintf(tp_filename, sizeof(tp_filename), "server_tpsetup_bytes_%d_%d_%d_%d.txt", ADVERSARY, SERVER_ID, CONST_T, CONST_N);
        ...
        size_t tp_bytes = sizeof(k_i[0]) + sizeof(rseed_i[0]) + sizeof(dseed_i[0])
                         + sizeof(aseedz_i[0]) + sizeof(rseedz_i[0]) + sizeof(dseedz_i[0]);
        fprintf(tp_file, "%zu\n", tp_bytes);
        fclose(tp_file);

        // Same idea for the RSS.Mul reindex step's genuine pairwise
        // party-to-party exchange (see mal_setup_reindex_bytes) — this
        // total is identical across all CONST_N server processes (each
        // locally simulates the full n-party setup), so writing it once
        // per (ADVERSARY,T,N) is enough; the wrapper takes any one file.
        char reindex_filename[256];
        snprintf(reindex_filename, sizeof(reindex_filename), "server_reindex_bytes_%d_%d_%d_%d.txt", ADVERSARY, SERVER_ID, CONST_T, CONST_N);
        FILE *reindex_file = fopen(reindex_filename, "w");
        if (reindex_file == NULL)
        {
            perror("fopen");
            exit(EXIT_FAILURE);
        }
        fprintf(reindex_file, "%zu\n", mal_setup_reindex_bytes());
        fclose(reindex_file);
