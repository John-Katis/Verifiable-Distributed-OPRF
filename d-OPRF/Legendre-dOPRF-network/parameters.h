...
#elif (SEC_LEVEL == 5)
    #define NBITS_FIELD     384
    #define NBYTES_FIELD    48
    #define WORDS_FIELD     6
#else
...
#define FIELD_PARAMS_H
    ...
    #elif (NBITS_FIELD == 384)
    // Gold-PRF modulus from the v-dOPRF paper's own Rust implementation:
    // p = 2^384 - 573*2^128 + 1 (see crates/bench/src/main.rs's p_hex) —
    // reused here per the paper's own §6 setup ("both PRFs are configured
    // at |p|=384 bits ... since machines share the same F_p"). R = 2^384.
    static const digit_t p[WORDS_FIELD]         = {0x0000000000000001, 0x0000000000000000, 0xFFFFFFFFFFFFFDC3, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF}; // Field order p
    static const digit_t Mont_one[WORDS_FIELD]  = {0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF, 0x000000000000023C, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000}; // R  =  2^{NBITS_PRIME} (mod p)
    static const digit_t pm1[WORDS_FIELD]       = {0x0000000000000000, 0x0000000000000000, 0xFFFFFFFFFFFFFDC3, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF}; // pm1 = p - 1
    static const digit_t R2[WORDS_FIELD]        = {0x0000000000000001, 0x0000000000000000, 0xFFFFFFFFFFFFFB86, 0xFFFFFFFFFFFFFFFF, 0x0000000000050288, 0x0000000000000000}; // R2 = (2^{NBITS_PRIME})^2 (mod p)
    static const digit_t iR[WORDS_FIELD]        = {0x000000000B36ACA5, 0x0000000000000000, 0xFFFFFFFFFFFFFB86, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFAFD76, 0xFFFFFFFFFFFFFFFF}; // iR =  R^(-1) (mod p)
    static const digit_t pp[WORDS_FIELD]        = {0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFDC2, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFAFD76, 0xFFFFFFFFFFFFFFFF}; // pp = -p^(-1) mod R
    static const digit_t ip[WORDS_FIELD]        = {0x0000000000000001, 0x0000000000000000, 0x000000000000023D, 0x0000000000000000, 0x0000000000050289, 0x0000000000000000}; // ip =  p^(-1) mod R
    static const digit_t Zero[WORDS_FIELD]      = {0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000}; // 0
    static const digit_t One[WORDS_FIELD]       = {0x0000000000000001, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000}; // 1
    #endif
#elif (PRIMES == ALT)
    #if (NBITS_FIELD == 64)
